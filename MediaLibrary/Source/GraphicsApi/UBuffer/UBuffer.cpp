...
namespace MultiStation{
        ...
        struct Allocator allocator        )
    {
        ...
        //check parameters
        MS_ASSERT(size , "The Size of the uniform buffer is defined zero !");
        MS_ASSERT(count, "The number of uniforms can not be zero !");
        MS_ASSERT(allocator.memalloc, "No Allocation function is given !!!");
        MS_ASSERT(allocator.memfree, "No Free Function is Given");
...
namespace MultiStation{
    ...
    void UBuffer::GetUniformNameByIndex(const ::uint32_t& index , char* name , ::uint32_t maxLength ){
        if (m_HasMoved){
            MS_ASSERT(0 , "This Uniforms Buffer has move its contents to other ubuffer !!!");
            return;
        }
        ...
        if (maxLength == 0){
            MS_ASSERT(0 , "Minimum length must be 1 ");
            return ;
        }
    ...
    bool UBuffer::AddUniform(UniformType type , char* name , ::uint32_t length , void* data ){
        ...
        if (((uintptr_t)alligmentPos + TypeSize) >= ((uintptr_t)m_UniformsMemory + m_Size)){
            MS_ASSERT(0 , "No Enough Uniform Data Space to add a new uniform !!!");
            return false;
        }
    ...
    ::uint32_t UBuffer::GetUniformIdByName(const char* name) const{
        ...
        if (m_SpecifiedUniforms == 0){
            MS_ASSERT(0 , "No Uniforms have added yet !!!");
            return 0xFFFFFFFF;
        }
    ...
    void* UBuffer::GetUniformPointerByName(const char* name) const{
        ...
        if (id == 0xFFFFFFFF){
            MS_ASSERT(0 , "Uniform Can't found !!!");
            return nullptr;
        }
    ...
    void UBuffer::RedirectUniformPointerByName(const char* name, void* newPointer) const {
		...
		if (newPointer == nullptr) {
			MS_ASSERT(0, "Can't redirect to nullptr !!!");
			return;
		}
