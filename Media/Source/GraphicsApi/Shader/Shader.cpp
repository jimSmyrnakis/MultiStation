...
namespace MultiStation{
    ...
    Shader::Shader(VShader& Vertex , FShader& Fragment , struct Allocator allocator){
        ...
        MS_ASSERT(m_Allocator.memalloc , "nullptr for memory allocator function !!!");
        if (m_Allocator.memalloc == nullptr)
            m_Allocator.memalloc = malloc;
        
        MS_ASSERT(m_Allocator.memfree , "nullptr for memory free function !!!");
        ...
        if (Vertex.HasCompiled() == false){
            MS_ASSERT(0 , "Vertex Shader has not comppiled !!!");
            m_HasProgram = false;
            return;
        }

        if (Fragment.HasCompiled() == false){
            MS_ASSERT(0, "Fragment Shader has not compiled !!!");
            m_HasProgram = false;
            return;
        }
    ...
    UBuffer* Shader::GetUniforms(void) { 
        MS_ASSERT(m_HasProgram , "No Program is Created !!!");
        MS_ASSERT(m_Uniforms , "Uniforms buffer is nullptr !!!");
        return m_Uniforms; 
    }

    #define CheckAllocation(pointer) if ((pointer) == nullptr) { MS_ASSERT(0 , "No Memory !!!"); return; }
    ...
    void Shader::FindUniforms(void){
        ...
        if (maxName ==0){
            MS_ASSERT(0 , "Something Is Wrong !@!!!!");
            return;
        }
    ...
    static void SetUniformDataBasedOnType(UniformType type , uint32_t location , void* data ){
        ...
        switch(type){
            ...
            default : MS_ASSERT(0 , "Undefined Shader Data Type !!!"); return ;
        }
    }
