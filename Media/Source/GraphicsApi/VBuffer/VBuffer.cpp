...
namespace MultiStation{
    ...
    void VBuffer::SetData(void* data , uint32_t size, uint32_t index){
        MS_ASSERT(size , "Vertex data have 0 size !!!");
        MS_ASSERT(data , "nullptr for data is not valid");
		MS_ASSERT(index < m_Layout.GetAttributeGroups().size(), "Index out of range for Vertex Buffer data set !");
	...
	void VBuffer::SetIndexBuffer(void* data, IBufferAttrib attrs) {
		MS_ASSERT(data, "nullptr for index buffer data is not valid");
		DestroyIndexBuffer();
		m_iBufferData = data;
