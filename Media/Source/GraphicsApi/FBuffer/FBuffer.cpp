...
namespace MultiStation{
    ...
    FBuffer::FBuffer(
        ...
        if (ActiveColorBuffers == 0){
            ...
            MS_ASSERT(0 , "Error -- Frame buffer with no color buffers ? Active Color Buffers are set to 1 !!!!");
            m_ActiveColorBuffers = 1;
        }

        if (ActiveColorBuffers > 16){
            MS_ASSERT(0 , "Error -- Max Color Buffer Attachments Supported are 8 !!! Set To max ");
    ...
    void FBuffer::InitBuffers(void) {
        ...
        for (int i = 0; i < m_ActiveColorBuffers; i++) {
            ...
            if (m_ColorBuffers[i] == nullptr) {
                MS_ASSERT(0, "Failed to allocate memory for Texture 2D for Frame Buffer Color Attachment !!!!");
            }
            uint32_t trash;
        ...
        switch(status){
            ...
            case GL_FRAMEBUFFER_UNDEFINED: MS_ASSERT(0 , "No Default Frame Buffer has Given from the Window System !!!"); break;
            case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT: 
            MS_ASSERT(0 , "Attachment's of the Frame Buffer are not Set right !!!");
            break;
            case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            MS_ASSERT(0 , "No image from Any Attachment of the Frame Buffer has Set right !!!");
            break;
            case GL_FRAMEBUFFER_UNSUPPORTED:
            MS_ASSERT(0 , "There is a no Supported Attachment for this Vendor !!!");
            break;
            default:
                MS_ASSERT(0 , "Frame Buffer Failed to set , for unknown cause !!!");
        }
    ...
    void FBuffer::Bind(void) const{
        if (m_BadState){
            MS_ASSERT(0 , "FrameBuffer is at  bad state :( !!!!");
            return;
        }
    ...
    void FBuffer::Unbind(void) const{
        MS_ASSERT(!m_BadState , "FrameBuffer is at  bad state :( !!!!");
