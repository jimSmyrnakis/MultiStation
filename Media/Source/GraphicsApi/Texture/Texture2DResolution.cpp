...
namespace MultiStation{
    ...
    void Texture2DResolution::SetWidth (const uint32_t&                   width ){
        if (width > GetMaxWidth()){
            m_Width = GetMaxWidth();
            m_BadW = true;
            MS_ASSERT(0 , "Width Greater that the maximum one !!!");
            return;
        }
        m_BadW = false;
        m_Width = width;
    ...
    void Texture2DResolution::SetHeight(const uint32_t&                   height){
        if (height > GetMaxHeight()){
            ...
            m_BadH = true;
            MS_ASSERT(0 , "Height Greater that the maximum one !!!");
            return;
        }
        m_BadH = false;
        m_Height = height;
    ...
    uint32_t                   Texture2DResolution::GetWidth (void) const{
        MS_ASSERT(!m_BadW , "Width has set to the maximum value ");
        return m_Width;
    }
    uint32_t                   Texture2DResolution::GetHeight(void) const{
        MS_ASSERT(!m_BadH , "Height has set to the maximum value ");
        return m_Height;
    }
